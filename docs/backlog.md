...
**Status:** ✅ RESOLVED 2026-08-20. Found the same day while running the Sprint-8 release gates.

`npm run format:check` (Prettier, CI step 2) was failing on already-committed files: `fabtraq-be`
149, `fabtraq-fe` 229, `fabtraq-shared` 47. `fabtraq-fe` builds every branch, so its CI was red on
every push; `fabtraq-be` and `fabtraq-shared` only build `main` and PRs into it, so theirs would
have landed on the merge PR.

How it hid: only `fabtraq-be`'s `verify` script chained `format:check`; `fabtraq-fe` and
`fabtraq-shared` ran `lint && typecheck && test && build` with no formatting step, so agents
verifying per-repo never executed it.

**Fixed by:**

1. `npm run format` in all three repos — one commit each, formatting only
   (shared `b099082` 47 files, be `488082c` 94 files, fe `44837cb` 154 files).
2. **Markdown excluded** in `fabtraq-be` and `fabtraq-fe` `.prettierignore`, matching
   `fabtraq-shared`, which already ignored `*.md`. The docs tree is mirrored byte-for-byte across
   all four repos plus the umbrella `docs/`; letting prettier rewrap markdown per-repo would have
   re-opened that drift immediately, and it could never converge while `e2e` has no prettier at all
   and the umbrella `docs/` is not a repo with npm scripts.
3. `format:check` prepended to `fabtraq-fe`'s and `fabtraq-shared`'s `verify` scripts, so the local
   gate now matches CI — that mismatch was the root cause.

4. **`fabtraq-be/docs/openapi.json` restored and added to `.prettierignore`** (`999d19c`). The
   format pass reformatted it; it is generated by `npm run openapi:emit` and **byte-compared by the
   B-004 CI drift gate** (`.github/workflows/ci.yml`), which would have gone red on the merge PR —
   green locally, red where it counts. Regenerated back to byte-identical with the pre-format spec.
   A formatter must not own generated output.

**Two gotchas for next time:**

- Prettier did **not** converge in one pass on `fabtraq-be` — three files were re-wrapped
  differently on the second run. Run `npm run format` twice, then `format:check`.
- After any repo-wide format, re-run the drift gate by hand:
  `npm run openapi:emit && git diff --exit-code docs/openapi.json`. `format:check` passing says
  nothing about it.

Gates re-run after formatting: shared 1166/1166 · be 770/770 unit (integration not re-run — it
truncates the live `fabtraq_dev`) · fe 1313/1313 serially plus `verify` with coverage; lint,
typecheck and build clean in all three. `e2e` has no prettier configuration and was untouched.
...
## B-040 — Purchase beam receipts have no vendor, so "Sourced From" shows "—"

**Status:** Open 2026-08-26. **Severity:** Medium.
**Context:** deferred out of the `sourcedFrom` workstream (shared 1.25.0) by owner decision.

The beam register's "Sourced From" column resolves the sourcing party from
`beam_receipt_items.out_item_id -> jw_challan_out_items -> jw_challans_out.job_worker_id`, which
exists only for `sizing_jw` beams. `in_house` renders the literal "In-house". **`purchase` has
...
(`beam.mapper.ts`, `beam-receipt.mapper.ts`) with a purchase arm. Nullable + no backfill keeps
existing purchase receipts valid; they keep rendering "—".

Do it in the same change: drop the now-dead `sizingName` from `beamResponseSchema` and
`beamReceiptItemResponseSchema`. It is free text no UI writes (**0 of 20 rows** in dev), was
`@deprecated` in shared 1.25.0 rather than removed because removal is a breaking major, and the
create-input schema (`beam-receipt.ts:55`) plus both DB columns were deliberately left alone.
Removing the response fields is the 2.0.0 item.

## B-041 — The seed creates a sizing_jw beam the API itself would reject

**Status:** Open 2026-08-26. **Severity:** Low.

`prisma/seed.ts:1435-1472` creates beam receipt seq 3 as `beamOrigin: 'sizing_jw'` with item
`BEAM-C-002` carrying **no `out_item_id`**, then registers its `Beam` row at :1462. The live API
cannot produce that state — `sizingJwBeamItemSchema.outItemId` is required on create (shared
`beam-receipt.ts:170`) and `beam-receipt.routes.ts` registers create + cancel only, with no update
path — so the seed bypasses the service and models something the domain forbids. `grep -n
outItemId prisma/seed.ts` returns exactly one hit (line 1193, Scenario 4).

Consequence: `BEAM-C-002` renders "—" in the "Sourced From" column forever, and is regenerated by
every `db:reset`. This is why the optional chaining in both mappers is load-bearing rather than
defensive.

Deliberately NOT fixed inside the `sourcedFrom` workstream: changing seed data that other e2e
